...
# Check agencies
async def check_agencies(x1='126.8281358', y1='37.5507676', x2='126.8603223', y2='37.5872323'):
    x1 = Decimal(x1)
    y1 = Decimal(y1)
    x2 = Decimal(x2)
    y2 = Decimal(y2)
...
async def check(area_list, vaccine_id, naver_cookies, driver, auto_progress):
...
def main(areas, vaccine, naver_cookies):
    ...
    while not result:
        ...
        result = asyncio.run(check(area_list, vaccine_id, naver_cookies, driver=driver, auto_progress=True))
        end_time = time.perf_counter()
        if result:
            print(f'{style.SUCCESS}{result}{style.ENDC}')
            input(f'Waiting for user input')
        else:
            wait_time = 0.3 + random.random() / 4.0
            print(f'- took: {end_time - start_time:.2f}s, wait: {wait_time:.2f}s')
            time.sleep(wait_time) # Check every 0.3 ~ 0.55 sec
